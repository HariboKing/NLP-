...
def deactivate_user_account(connection: sqlite3.Connection, user_id: int) -> None:
    ...
    if user["is_platform_admin"]:
        raise ValueError("Het owner-account kan niet worden gedeactiveerd.")

    deactivated_at = utc_now_iso()
    safe_stamp = (
        deactivated_at.replace("-", "")
        .replace(":", "")
        .replace("T", "")
        .replace("+", "")
        .replace(".", "")
    )
    replacement_email = f"deactivated-{user_id}-{safe_stamp}@deactivated.local"

    connection.execute("DELETE FROM sessions WHERE user_id = ?", (user_id,))
    connection.execute("DELETE FROM assignment_targets WHERE target_user_id = ?", (user_id,))
    connection.execute("DELETE FROM student_group_members WHERE user_id = ?", (user_id,))
    connection.execute("DELETE FROM memberships WHERE user_id = ?", (user_id,))
    connection.execute("UPDATE users SET managed_by_user_id = NULL WHERE managed_by_user_id = ?", (user_id,))
    connection.execute(
        """
...
def get_plan_by_code(connection: sqlite3.Connection, code: str) -> sqlite3.Row | None:
    return connection.execute(
...
def get_active_subscription(connection: sqlite3.Connection, user_id: int) -> sqlite3.Row | None:
    return connection.execute(
        """
...
def set_active_subscription(connection: sqlite3.Connection, user_id: int, plan_code: str) -> sqlite3.Row:
...
def ensure_public_free_subscription(connection: sqlite3.Connection, user_id: int) -> sqlite3.Row:
...
def activate_public_paid_subscription(connection: sqlite3.Connection, user_id: int) -> sqlite3.Row:
    return set_active_subscription(connection, user_id, "public_paid")


def get_plan_model_access_slugs(connection: sqlite3.Connection, plan_id: int) -> set[str]:
    return {
        row["model_slug"]
        for row in connection.execute(
...
def get_plan_module_access_ids(connection: sqlite3.Connection, plan_id: int) -> set[int]:
    return {
        int(row["module_id"])
        for row in connection.execute(
...
def export_account_access_rows(
    connection: sqlite3.Connection,
    ...
    if organization_id is not None:
        where_clause = "WHERE users.is_active = 1 AND memberships.organization_id = ?"
        parameters = (organization_id,)
    else:
        where_clause = "WHERE users.is_active = 1"
    return connection.execute(
        f"""
...
def sync_subscription_catalog(connection: sqlite3.Connection) -> None:
    created_at = utc_now_iso()
    plans = [
        {
            "code": "public_free",
...
def backfill_public_subscriptions(connection: sqlite3.Connection) -> None:
    free_plan = get_plan_by_code(connection, "public_free")
    if not free_plan:
        return
